...
from orbit_core import (
    seed_everything,
    get_device,
    CanonicalScaler, CanonicalScaleConfig,
    OrbitOperatorDataset,
    OrbitPINN,
    save_checkpoint,
    load_checkpoint,
    time_derivative,
...
def _curriculum_tmax_canonical(cfg: TrainConfig, epoch: int, *, dataset: OrbitOperatorDataset, scaler: CanonicalScaler) -> float:
...
def _eval_data_terms(
    ...
    *,
    ...
    for bt, bic, by in loader:
        ...
        if need_grad:
            drdt_v = time_derivative(yp[:, 0:3], bt)
...
def train(cfg: TrainConfig) -> str:
    ...
    try:
        scaler = CanonicalScaler(CanonicalScaleConfig())
        dataset = OrbitOperatorDataset(cfg.dataset_csv, scaler, dtype=dtype)
        ...
        for epoch in range(adam_epochs):
            ...
            for t, ic, y_true in loader:
                ...
                drdt_pred = time_derivative(y_pred[:, 0:3], t)
        ...
        if cfg.use_lbfgs and int(cfg.lbfgs_max_iter) > 0:
            ...
            def closure():
                ...
                drdt_p = time_derivative(yp[:, 0:3], bt)
