...
def _get_initial_state(cfg: EvalConfig) -> np.ndarray:
    ...
    a = r_p / (1.0 - e)
    y0 = coe_to_eci_state(a=a, e=e, inc=inc, raan=raan, argp=argp, nu=nu, mu=float(MU_EARTH)).astype(np.float64)

    if y0.shape != (6,) or (not np.all(np.isfinite(y0))):
        raise RuntimeError("coe_to_eci_state geçersiz state üretti.")
    return y0
...
def _build_scaler_from_extra(extra: Dict[str, Any]) -> CanonicalScaler:
    scaler_cfg = CanonicalScaleConfig()
